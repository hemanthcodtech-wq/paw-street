  ...
  const handleRequestForgotPassword = async (e) => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        setErrorMessage(res?.message || 'No administrator account found with that email.');
      }
    } catch (err) {
            ...
            <>
              <form onSubmit={handleLogin} className="space-y-4 text-xs">
                ...
                >
                  ...
                  <span>{isLoading ? 'Authenticating Admin...' : 'Enter Admin Command Center'}</span>
          ...
          {step === 'forgot_email' && (
            <form onSubmit={handleRequestForgotPassword} className="space-y-4 text-xs">
              <div className="text-center space-y-1 pb-1">
                ...
                <h3 className="font-heading font-bold text-sm text-slate-900">Admin Password Recovery</h3>
                <p className="text-slate-500 text-[11px]">
                  We will dispatch a secure 6-digit passcode to your registered admin email via Nodemailer.
                </p>
              </div>
          ...
          {step === 'forgot_otp' && (
            <form onSubmit={handleResetPasswordSubmit} className="space-y-4 text-xs">
              ...
              <button
                ...
                className="w-full py-3 bg-[#FFB703] hover:bg-[#E5A015] text-slate-950 font-black rounded-xl shadow-xs transition-all flex items-center justify-center gap-2"
              >
                <span>{isLoading ? 'Updating Password...' : 'Save Password & Enter Admin'}</span>
                <ArrowRight className="w-4 h-4" />
              </button>
